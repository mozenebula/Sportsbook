...
const SwapPage = () => {
  ...
  const [buttonText, setButtonText] = useState("Add Liduidity")
  ...
  const [desApproved, setDesApproved] = useState(false)
  const [buttonDisabled, setButtonDisabled] = useState(true)


  
  const url = 'https://mainnet.infura.io/v3/b77d80e759ce4c0e94276a0afa6ea0fa';

  const usdt_constract = "0xdAC17F958D2ee523a2206206994597C13D831ec7"
  ...
  const onItemSelected = async(item: CoinInfo) => {
    ...
    const balance = await balanceOf(item.contract)
  ...
  const handleClick = async () => {
    if (window.ethereum&&window.ethereum.isConnected) {
      ...
      if(sourceValue > 0 && sourceValue < sourceBalance && !sourceApproved) {
        ...
        if (sourceFlag) setButtonText(`Approve ${des.name}`)
      }
      
      if(desValue > 0 && desValue < desBalance && !desApproved) {
        const desFlag = await approve(des.contract, RouterAddress, desValue);
        setDesApproved(desFlag)
        if (desFlag) setButtonText("Add Liquidity")
      }

      if (sourceValue > 0 && desValue > 0 && sourceApproved && desApproved) {
        addLiquidity(source.contract, des.contract, sourceValue, desValue, window.ethereum.selectedAddress)
      }

      const provider = new ethers.BrowserProvider(window.ethereum);
      const contract = new Contract(RouterAddress, ABI[RouterAddress], provider);
      // approve(source.contract, BigInt(100))
      
      // 现在你可以使用 provider 进行以太坊操作
    } else {
  ...
  const handleInputChange = (e, position) => {
    ...
    if (sourceValue > 0 && sourceCardstyle == null && !sourceApproved) {
